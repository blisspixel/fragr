...
| Solo run ownership and continues | `server/src/mission/recovery.rs` owns entry capture and explicit retry; reuse encounter reset. Local campaign opts in, dedicated development parties retain their rules. Never rewind ticks, input sequence or inventory revisions. Restore camera facing from the retry snapshot before sending new aim. Disk saves and persistent statistics remain separate work. |
