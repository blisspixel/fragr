...
- `tools/` - `solo_scrap.sh`, screenshot capture, `godot_check.sh` (headless client checks), `audiogen/` (developer-only ElevenLabs sound and music generation, Rust), `licenses/` (third-party notices for packaged `fragr-server`, Rust), and `playtest/` (the agent playtest harness that runs in CI).
...
| Server CLI, tracing, tick loop | `server/src/main.rs` (`--bind`, `--bots`, `--map`, `--map-rotate`, `--ban-list`, `--allow-list`) |
