...
| Mission use, gates and shared departure | `server/src/mission.rs`, `protocol/mission.rs`, `maps/authored/mission.rs`. The encounter lifecycle owns party reset. Precompute gate worlds before readiness; resend MapInfo before mission state. Shared wire control: `mission/controller.rs`; client validation/UI: `mission_state.gd`, `mission_hud.gd`. |
