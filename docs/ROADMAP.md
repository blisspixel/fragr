...
- Join tickets: `FRAGR_JOIN_SECRET` makes a hosted server refuse a human or agent hello that lacks a short HMAC ticket. Spectators can still watch. No secret keeps local and LAN hello open. A dropped player still starts a new session.
...
Admission is moving beside these mission rungs. Frame caps, per-address caps, the inbound budget, `GET /status`, and the app match line are on main. Join tickets now lock human and agent hello when `FRAGR_JOIN_SECRET` is set, and watch stays open. Next on that seam is a reconnect that keeps the same pawn, then a measured spectator fan-out before any higher connection cap. Those rungs do not close the M01 quality gate. The next mission rung is still two readable enemies.
