...
Admission sits beside the mission rungs and does not close the M01 quality gate. v0.35.0 through v0.39.0 shipped frame caps, per-address caps, the inbound budget, `GET /status`, the app match line, join tickets, and a ten-second pawn resume. v0.40.0 lets the mission card leave after the introduction. v0.41.0 is the player-facing gun names. v0.42.0 is weapon cycling and four README stills. The Clerk and the Sweeper no longer share one outline in the unshaded atlases. A live Recall Notice run on 2026-09-22 confirmed the boot menu, host `127.0.0.1:6767`, fists then the pistol, the `+24 BULLETS` pickup, and the mouse wheel switching between those two guns. The intro card left, then was visible again later in that same run, so it is not yet proven to stay gone. Key 1 did not leave the pistol in that run. Next on the server seam is a measured spectator fan-out before any higher connection cap. The next mission rung is the optional supply detours, then automated mission review; human acceptance remains near 1.0. Paid enemy sheets were not bought.

Automated Jev playtests run alongside these rungs. Durable paid-call reservations shipped in [#218](https://github.com/blisspixel/fragr/pull/218). The free first-person watcher is implemented ([plan](plans/agent-first-person-watch.md)). Next, make Jev's mission decisions and outcomes campaign-aware before any capped OpenRouter trial. Its results can expose route and presentation defects, but cannot claim a fresh-player comprehension result. Human acceptance sessions are deferred until the game is much closer to 1.0; automated evidence should continue to improve M01 and later missions in the meantime.
...
**4. Optional detours, then secrets.** First make the confiscation alcove and maintenance overlook readable walking detours with optional supplies, and prove that ordinary routes never claim them. This static slice is in [review](plans/m01-optional-supply-detours.md). Neither holds the objective, the ward name, or a required gun. After the imperfect-aim supply check, the level plan's Shiv can occupy the alcove once a pool-less melee grant exists. A moving wall panel waits on the door seam. Finding either detour is not part of the clear in rung 1.
...
**10. LAN, then the exposed server, then 1.0.** A two-machine session on the predicted sim, with humans, rule bots, and an agent. Then join control, rate and size caps, protocol rejection, reconnect, a status endpoint, and desktop binaries that boot to a fight. Measure WebSocket under that load before any UDP spike. The latest six-map mixed-roster assertion passed, but its reports show 3, 4, 3 and 1 spawn deaths on maps 3 through 6 at the tested seeds; resolve that map-quality gap before the LAN exit. The exposed-server exit is a public week. Version 1.0 is the full agreed run, the measured control numbers, no placeholder art, the fun bar on a recorded multiplayer session, a soak, and those binaries. The Host bumper, the podium, and a killfeed beat every thirty seconds are the multiplayer exit. They are not requirements inside M01, which ends at a lift. Cloud apply stays plan-only until that public week is real.
