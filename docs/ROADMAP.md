...
**6. Finish M02 on the durable run and objective seams.** The versioned local run file shipped in v0.45.0. It resumes M01 at mission entry without refilling continues or rewinding live state, and records departure without pretending the campaign is over. The [M02 foundation](plans/m02-objective-gates.md) now validates authored objectives and precomputed gate worlds, advances them on the server, and publishes optional mission state to Rust readers. M01 content and capability 8 remain regression fixtures. A bundled ward graybox (gallery, stair, antechamber, ward, processing floor, loading dock) now carries the five objectives and three visible gates, each opened by a switch or arrival right beside it and marked by matching lamps that turn from red to green, and `--local-mission persons_unknown` serves it as a development child with no run file. Seeded tests walk a solo human and a solo agent through all five objectives in order with the shared wire controller, prove the shut gates block the early route, and prove a wipe restores the first objective and closed gates. Latch is a stub arrival, the Jammer a stub switch, and the graybox has no enemies. Controls follow the new few-puzzles, readable-without-English rule in [`MAP-DESIGN.md`](MAP-DESIGN.md). Next, connect the Godot objective display, gate presentation and a development menu entry, then inspect first-person captures. This comes before a new enemy or weapon because a correct server transition alone cannot make the rescue legible or playable. Then add Latch's story-controlled release, the visible Jammer projectile, the Scatter pickup that already exists in combat, and M01-to-M02 run carry with explicit save migration tests. M02 is not complete until the route, retry, presentation and encounter evidence pass; Latch is not a second player.
