...
Admission sits beside the mission rungs and does not close the M01 quality gate. v0.35.0 through v0.39.0 shipped frame caps, per-address caps, the inbound budget, `GET /status`, the app match line, join tickets, and a ten-second pawn resume. v0.40.0 lets the mission card leave after the introduction. v0.41.0 is the player-facing gun names. v0.42.0 is weapon cycling and four README stills. The Clerk and the Sweeper no longer share one outline: the bot is wider, and the clerk's aim clears the shoulder while the sweeper's rifle stays inside its pauldrons. Next on the server seam is a measured spectator fan-out before any higher connection cap. The next mission rung is an optional secret, then a fresh-player review. Rungs 1 through 3 are done. Paid enemy sheets were not bought.
