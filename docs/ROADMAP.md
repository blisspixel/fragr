...
Automated Jev playtests run alongside these rungs, not as a substitute for their acceptance gates. First make paid calls crash-safe under one durable shared ledger ([plan](plans/jev-budget-reservations.md)); then prove the free controller and first-person watcher before a capped OpenRouter trial. Its results can expose route and presentation defects, but cannot claim a fresh-player comprehension result.
