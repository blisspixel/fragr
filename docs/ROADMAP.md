...
Story between levels is a short audio slideshow on the scene player that now runs the M01 opening: pixel stills, voiced lines and captions, skippable, falling back to the text page when an asset is missing. Stills through `tools/spritegen` and voices through `tools/audiogen` wait for frozen wording and Nick's go per batch. Higgsfield video, including Seedance 2.5 character cutscenes, waits until the playable campaign is built; it would reuse the same shots. [`plans/campaign-scenes.md`](plans/campaign-scenes.md) holds the scene list, costs and gates.
