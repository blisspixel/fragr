...
- 2026-09-24, client follow-up on `fix/client-kick-reasons`: `client/scripts/net_client.gd` maps `idle_timeout`, `rate_limited`, `malformed`, `address_banned` and `address_not_allowed` to their own localized message (keys in `client/i18n/world.en.po`), shown through the existing `server_error` path used for `join_rejected`. `rate_limited`, `malformed`, `address_banned` and `address_not_allowed` are now hard stops that skip the client's automatic resume attempt, since the server has already removed the pawn and its token; `idle_timeout` keeps the existing one resume attempt, same as a plain drop. Headless coverage in `client/scripts/test_kick_reasons.gd`.
- Still open: a client send-rate throttle. The client sends one action per rendered frame and renders uncapped by default, so a high refresh-rate client can send well above the server's 20 Hz tick. A naive throttle is not safe as a quick change: `pending_jump`, `pending_reload`, `pending_interact` and the weapon-swap latch in `game_manager.gd`'s human input block are edge-triggered flags that are unconditionally cleared once `send_action` is called each frame; skipping a send on a throttled frame without also holding those latches open would silently drop a jump, reload, interact or weapon swap press, changing feel and risking the jump-buffering contract in `docs/protocol.md`. A correct version needs the latch-clear to happen only on the frame that actually flushes to the network, not on every rendered frame, which is more than this follow-up's scope; tracked here for the next rung. Behind a reverse proxy the lists and per-address caps see only the proxy address; there is no forwarded-address support. There is no inbound byte-rate budget beyond the 64 KiB frame cap.
