...
| [`readable-arsenal.md`](./readable-arsenal.md) | **in flight** | HUD names are Pistol, Rifle, Shotgun, and Railgun. Sniper, rocket, grenade, and both mines are campaign finds and are not implemented. |
...
| [`public-server-hardening.md`](./public-server-hardening.md) | **in flight** | v0.35.0 shipped frame caps, connection caps, and the inbound budget. `GET /status` is the host probe. HMAC join tickets lock human and agent hello when `FRAGR_JOIN_SECRET` is set. A drop can resume the same pawn ([`resume-pawn.md`](./resume-pawn.md)). TLS remains. |
