...
| [`public-server-hardening.md`](./public-server-hardening.md) | **in flight** | v0.35.0 shipped frame caps, connection caps, and the inbound budget. `GET /status` is the host probe. HMAC join tickets lock human and agent hello when `FRAGR_JOIN_SECRET` is set. Reconnect and TLS remain. |
