...
func torso(root: Node3D, bot: bool) -> void:
	...
	var cloth: Color = GREEN if not bot else STEEL
	...
	for side: float in [-1.0,1.0]:
		...
		part(root,Vector3(side*0.109,chest+0.04,0.211),Vector3(0.16,0.20,0.012),BONE.darkened(0.075),Vector3(0,side*8,side*-3))
		part(root,Vector3(side*0.22,hip+0.12,0.085),Vector3(0.08,0.115,0.14),GREEN.darkened(0.25))
		part(root,Vector3(side*0.225,hip+0.16,0.161),Vector3(0.065,0.023,0.012),BONE.darkened(0.22))
	...
	if not bot:
		...
		for side: float in [-1.0,1.0]:
			...
			oval(root,Vector3(side*0.133,1.670,-0.039),Vector3(0.050,0.14,0.126),BONE)
		helmet(root)
	else:
		...
		for y: float in [1.16,1.23,1.30,1.37]:
			part(root,Vector3(0,y,-0.325),Vector3(0.28,0.025,0.02),BONE.darkened(0.3))
	if not bot:
		part(root,Vector3(0,1.24,-0.146),Vector3(0.32,0.28,0.041),GREEN.darkened(0.24))
		for side: float in [-1.0,1.0]:
			part(root,Vector3(side*0.123,1.22,-0.173),Vector3(0.038,0.35,0.017),STEEL)
			part(root,Vector3(side*0.122,1.37,-0.187),Vector3(0.06,0.043,0.014),BONE.darkened(0.28))
		...
		part(root,Vector3(0,1.18,0.189),Vector3(0.035,0.25,0.02),GREEN.darkened(0.35))
	...
	part(root,Vector3(-0.102,1.285,0.22),Vector3(0.079,0.047,0.01),GREEN)
	for x: float in [-0.129,-0.108,-0.092,-0.077]:
		part(root,Vector3(x,1.287,0.227),Vector3(0.007,0.025,0.007),BONE.darkened(0.2))
...
func leg(root: Node3D, bot: bool, side: float, hip: Vector3, knee: Vector3, ankle: Vector3) -> void:
	var cloth: Color = STEEL if bot else GREEN
	if bot:
	...
	if bot:
		...
		limb(root,knee+Vector3(side*0.07,-0.08,-0.01),ankle+Vector3(side*0.07,0.02,-0.01),0.034,0.034,BONE.darkened(0.3))
	else:
		sleeve(root,knee,ankle,0.205,0.24,cloth)
		limb(root,ankle,ankle.lerp(knee,0.43),0.185,0.22,STEEL)
	var pad: MeshInstance3D = part(root,knee+Vector3(0,0,0.108),Vector3(0.16,0.15,0.073),BONE)
	pad.rotation_degrees.x = -9.0
	part(root,knee+Vector3(0,0,0.15),Vector3(0.105,0.083,0.009),BONE.darkened(0.2))
	part(root,ankle+Vector3(0,-0.04,0.07),Vector3(0.22,0.16,0.37),INK)
	part(root,ankle+Vector3(0,0.01,0.17),Vector3(0.205,0.11,0.15),BONE if bot else STEEL)
	part(root,ankle+Vector3(0,-0.105,0.06),Vector3(0.23,0.037,0.39),INK)
	if not bot:
		var pouch: MeshInstance3D = part(root,hip.lerp(knee,0.42)+Vector3(side*0.055,0,0.14),Vector3(0.11,0.14,0.05),GREEN.lightened(0.12))
